...
void simulate_fifo(EventQnodePtr eventsQheadPtr,
		   EventQnodePtr eventsQtailPtr)
{
    ...
    out_file = fopen("fifo.trace", "w");
    if (out_file == NULL) {
	fprintf(stderr, "Could not open fifo.trace for writing!\n");
	return;
    }
	...
	if (currentEvent.type == 0) {	//Arrival logic
	    ...
	    enqueue(&rqheadPtr, &rqtailPtr, task, FIFO);
	}
